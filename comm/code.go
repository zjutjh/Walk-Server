...
var (
	...
	CodeAdminLoginTooFrequently      = kit.NewCode(200321, "登录失败次数过多，请稍后再试")
	CodeTeamLostLocked               = kit.NewCode(200322, "团队刚打过卡，不可标记失联")
	CodeNonStudentRegister           = kit.NewCode(200323, "非学生不可注册学生账号")
	CodeNonTeacherRegister           = kit.NewCode(200324, "非教职工不可注册教职工账号")
	CodePeopleInfoWrong              = kit.NewCode(200325, "信息错误，无法登录")
	CodeCannotAddSelf                = kit.NewCode(200326, "不能添加自己")
	CodeTeacherCannotJoinStudentTeam = kit.NewCode(200327, "教师不可加入学生团队")
	CodeCannotChangeCaptain          = kit.NewCode(200328, "非学生队不可将队长转让给学生")
	CodeJoinTeamFailed               = kit.NewCode(200329, "加入团队失败")
	CodeCannotLeaveTeam              = kit.NewCode(200330, "队长不可离开团队")
	CodeLeaveTeamFailed              = kit.NewCode(200331, "离开团队失败")
	CodeTeamNotAllowMatch            = kit.NewCode(200332, "团队不允许匹配")
	CodeNotFindTeams                 = kit.NewCode(200333, "未找到符合要求的团队")
	CodeCannotRemoveSelf             = kit.NewCode(200334, "不能移除自己")
	CodeRemoveFailed                 = kit.NewCode(200335, "移除成员失败")
	CodeTeamNotSubmitted             = kit.NewCode(200336, "团队未提交，无法回滚")
	CodeNotInRegisterTime            = kit.NewCode(200337, "当前时间不在报名时间内")
